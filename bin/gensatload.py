...
#  Inputs:
#
#      - Input file (${GENSATLOAD_INPUTFILE}) containing a list of
#        EntrezGene IDs
#
#  Outputs:
#
#      - BCP file (${GENSATLOAD_ACC_BCPFILE}) for the ACC_Accession table
#        containing the associations between EntrezGene IDs and markers
#
#      - Discrepancy report (${GENSATLOAD_RPTFILE})
#
#  Exit Codes:
#
...
#
# Purpose: Create the discrepancy report for the GENSAT data.
# Returns: Nothing
# Assumes: Nothing
# Effects: Nothing
# Throws: Nothing
...
def createReport ():
    ...
    fpRptFile.write(24*' ' + 'GENSAT Discrepancy Report' + NL)
    ...
    #
    cmds.append('select gs.entrezgeneID ' + \
                'into #not_in_mgi ' + \
                'from tempdb..GENSAT_Data gs ' + \
                'where not exists (select 1 ' + \
                                  'from ACC_Accession a ' + \
                                  'where a.accID = gs.entrezgeneID and ' + \
    ...
    cmds.append('select gs.entrezgeneID ' + \
                'into #many_marker ' + \
                'from tempdb..GENSAT_Data gs, ACC_Accession a ' + \
                'where gs.entrezgeneID = a.accID and ' + \
                      'a._MGIType_key = ' + str(markerMGITypeKey) + ' and ' + \
                      'a._LogicalDB_key = ' + str(egLogicalDBKey) + ' ' + \
                'group by gs.entrezgeneID ' + \
                'having count(*) > 1')
    ...
    cmds.append('select gs.entrezgeneID ' + \
                'into #many_eg ' + \
                'from tempdb..GENSAT_Data gs, ACC_Accession a, #markers m ' + \
                'where gs.entrezgeneID = a.accID and ' + \
                      'a._MGIType_key = ' + str(markerMGITypeKey) + ' and ' + \
...
#
# Purpose: Create the bcp file for the GENSAT associations.
# Returns: Nothing
# Assumes: Nothing
# Effects: Nothing
# Throws: Nothing
...
def createBCPFile ():
    ...
    #
    cmds = []
    cmds.append('select gs.entrezgeneID, a._Object_key "markerKey" ' + \
                'from tempdb..GENSAT_Data gs, ACC_Accession a ' + \
                'where gs.entrezgeneID = a.accID and ' + \
                      'a._MGIType_key = ' + str(markerMGITypeKey) + ' and ' + \
                      'a._LogicalDB_key = ' + str(egLogicalDBKey) + ' ' + \
                'order by gs.entrezgeneID')
